...
| ④ 外壳骨架结构不同 | Neon 左侧栏+玻璃面板 vs CRT 盒状 `[ JSON ]` 菜单 | **皮肤级布局组件**（shell） |
| ⑤ 内容组件内部结构差异 | 某工具两个皮肤下控件摆放差异 | 先 CSS，实在不行才用 `useSkin()` 最小分支 |

**外壳与内容分层约定：**
- **外壳（Shell）允许分叉**：`App.vue` 按当前皮肤渲染对应的布局组件（如 `NeonShell` / `CrtShell`），每个 shell 是完整布局模板 + 对应 scoped 样式。改动 shell 不影响工具。
- **内容（工具页）强制共享**：所有工具组件放在 shell 的 `<RouterView>` 内，只消费令牌，**禁止感知皮肤**。切换 shell 不影响工具的 Pinia 状态。
